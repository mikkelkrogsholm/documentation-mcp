"""next-intl documentation module implementation."""
...
from src.modules.nextintl import config


class NextIntlModule(BaseModule):
    """Fetches next-intl documentation from next-intl.dev."""
    ...
    def get_doc_urls(self) -> list[NavLink]:
        ...
        for url_elem in root.findall(".//sm:url", ns):
            ...
            if loc is not None and loc.text:
                ...
                parsed = urlparse(url)
                if not parsed.path.startswith(config.DOCS_PATH_PREFIX):
                    continue
                ...
                # Calculate depth
                depth = len(segments) - 2  # -2 for "docs" segment and page
                depth = max(0, depth)
