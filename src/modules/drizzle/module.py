"""Drizzle ORM documentation module implementation."""
...
from src.modules.drizzle import config


class DrizzleModule(BaseModule):
    """Fetches Drizzle ORM documentation from orm.drizzle.team."""
    ...
    def get_doc_urls(self) -> list[NavLink]:
        ...
        # Pattern matches full URLs starting with https://orm.drizzle.team/docs/
        pattern = r"(https://orm\.drizzle\.team/docs/[^\s\)\"\']+)"
        ...
        for url in unique_urls:
            ...
            # e.g., /docs/get-started/postgresql-new -> "Postgresql New"
            segments = path.strip("/").split("/")
            title = segments[-1].replace("-", " ").replace("_", " ").title()

            # Calculate depth from path structure
            # /docs/overview = depth 0, /docs/get-started/foo = depth 1
            depth = len(segments) - 2  # -2 for "docs" and the page itself
            depth = max(0, depth)
